...
- **Values of the wrong type or shape** (e.g. `port = 70000`, `backend = "mongodb"`, or a Redis `url` without a `redis://` or `rediss://` scheme) are fatal: `chel` prints a clear message naming the offending key and exits with code 1.
