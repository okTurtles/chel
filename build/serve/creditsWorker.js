...
var syncServerTime = async function() {
  ...
  if (requestTimeElapsed - startTime2 > 8e3) {
  ...
  wallBase = serverTime + (requestTimeElapsed - startTime2) / 2 + // Also take into account the time elapsed between `requestTimeElapsed`
...
esm_default("sbp/selectors/register", {
  "worker/computeCredits": async () => {
    ...
    billableEntities && await Promise.all(billableEntities.split("\0").map(async (billableEntity) => {
      ...
      updateCredits(billableEntity, "charge", size).catch((e2) => {
        console.error(e2, "[creditsWorker] Error computing balance", billableEntity);
      });
    }));
