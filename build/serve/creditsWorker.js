...
var updateCredits = async (billableEntity, type, amount) => {
  ...
  await default4("okTurtles.eventQueue/queueEvent", granularHistoryKey, async () => {
    const now = /* @__PURE__ */ new Date();
    const date = now.toISOString();
    const granularHistoryList = await default4("chelonia.db/get", granularHistoryKey, { bypassCache: true }) ?? "[]";
    ...
    if (type === "charge") {
      ...
      await default4("chelonia.db/set", `_private_ownerPicocreditBalance_${billableEntity}`, balancePicocreditAmount);
    } else if (type === "credit") {
      ...
      await default4("chelonia.db/set", `_private_ownerPicocreditBalance_${billableEntity}`, newBalanceStr);
    } else {
    ...
    if (lastCoarseSyncIdx >= GRANULAR_ENTRIES_PER_COARSE_ENTRIES || lastCoarseSyncIdx < 0) {
      ...
      const coarseHistoryList = await default4("chelonia.db/get", coarseHistoryKey, { bypassCache: true }) ?? "[]";
      ...
      await default4("chelonia.db/set", coarseHistoryKey, JSON.stringify(coarseHistory));
    }
    await default4("chelonia.db/set", granularHistoryKey, JSON.stringify(granularHistory));
  });
};
default4("okTurtles.eventQueue/queueEvent", readyQueueName, () => setTimeout(default4, CREDITS_WORKER_TASK_TIME_INTERVAL, "worker/computeCredits"));
default4("sbp/selectors/register", {
  "worker/computeCredits": async () => {
    const billableEntities = await default4("chelonia.db/get", "_private_billable_entities", { bypassCache: true });
    billableEntities && await Promise.all(billableEntities.split("\0").map(async (billableEntity) => {
      const sizeString = await default4("chelonia.db/get", `_private_ownerTotalSize_${billableEntity}`, { bypassCache: true });
    ...
    setTimeout(default4, CREDITS_WORKER_TASK_TIME_INTERVAL, "worker/computeCredits");
  }
});
