...
import { base58btc, blake, CID, Buffer } from '../../deps.ts'
...
// Blake2b hash functions from multiformats
const blake2b256 = blake.blake2b256
const blake2b256stream = blake.blake2b256stream
...
export const parseCID = (cid: string): ParsedCID => {
  ...
  const parsed = CID.parse(cid, base58btc) as ParsedCID
  if (
    parsed.version !== 1 ||
...
export const maybeParseCID = (cid: string): ParsedCID | null => {
  try {
    return parseCID(cid)
  } catch {
...
export const getSubscriptionId = async (subscriptionInfo: PushSubscriptionInfo): Promise<string> => {
