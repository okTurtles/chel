  ...
  try {
    ...
    if (subscription) {
      ...
      if (server.subscribersByChannelID) {
        subscription.subscriptions.forEach((channelID: string) => {
        ...
        return function (this: PushSubscriptionInfo) {
          ...
          if ((count | 0) === 0) {
            if (!salt) {
              ...
              // TypeScript: this.keys.auth is browser-generated auth salt
              salt = Buffer.from(this.keys.auth, 'base64url')
            }
            if (!uaPublic) {
              // `this.keys.p256dh` is a public key that is browser-generated
              // TypeScript: this.keys.p256dh is browser-generated public key
...
export const pushServerActionhandlers: PushServerActionHandlers = {
  ...
  async [PUSH_SERVER_ACTION_TYPE.STORE_SUBSCRIPTION] (payload) {
    ...
    if (applicationServerKey) {
      ...
      // TypeScript: Convert base64 application server key to base64url format
    ...
    try {
      ...
      socket.subscriptions?.forEach((channelID: string) => {
        // TypeScript: Adding channel ID to subscription wrapper
