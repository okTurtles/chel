...
interface GranularCreditEntry extends GranularHistoryEntryBase {
  type: 'credit';
  picocreditAmount: string; // BigInt string
}

type GranularHistoryEntry = GranularChargeEntry | GranularCreditEntry;
...
const updateCredits = async (billableEntity: string, type: 'credit' | 'charge', amount: number) => {
  ...
  await sbp('okTurtles.eventQueue/queueEvent', granularHistoryKey, async () => {
    ...
    const granularHistory: GranularHistoryEntry[] = JSON.parse(granularHistoryList)
    ...
    if (lastCoarseSyncIdx >= GRANULAR_ENTRIES_PER_COARSE_ENTRIES || lastCoarseSyncIdx < 0) {
      ...
      const coarseHistory: CoarseHistoryEntry[] = JSON.parse(coarseHistoryList)
      ...
      const { periodStart, charges, credits, periodSize, totalPeriodLength } = granularHistory.slice(0, lastCoarseSyncIdx < 0 ? granularHistory.length : lastCoarseSyncIdx).reduce((acc: { charges: bigint; credits: bigint; periodStart: string; periodSize: number; totalPeriodLength: number }, entry: GranularHistoryEntry) => {
