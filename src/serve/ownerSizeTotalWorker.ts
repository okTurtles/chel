...
const updatedSizeList = new Set<string>()
const updatedSizeMap = new Map<string, number>()
const cachedUltimateOwnerMap = new Map<string, string>()
...
sbp('sbp/selectors/register', {
  ...
  'backend/server/computeSizeTaskDeltas': async function () {
    ...
    const ultimateOwners = new Map<string, [number, Set<string>]>()
    ...
    await Promise.all(deltaEntries.map(async ([contractID, delta]: [string, number]) => {
    ...
    // the temporary index.
    await Promise.all(Array.from(ultimateOwners).map(async ([id, [totalDelta, contributingResources]]: [string, [number, Set<string>]]) => {
  ...
  'backend/server/computeSizeTask': async function () {
    ...
    const ultimateOwners = new Map<string, Set<string>>()

    // Phase 1: Find the ultimate owner for each resource ID needing recalc
    await Promise.all(resourcesToRecalculate.map(async (contractID: string) => {
      // --- Potentially Slow Owner Lookup Loop ---
    ...
    // total size.
    await Promise.all(Array.from(ultimateOwners).map(async ([ownerID, contractIDs]: [string, Set<string>]) => {
