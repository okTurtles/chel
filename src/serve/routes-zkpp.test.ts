  ...
  async fn (t: Deno.TestContext) {
    ...
    try {
      ...
      await t.step('POST /zkpp/register step 1 with form-urlencoded body', async () => {
        ...
        const res = await fetch(`${baseURL}/zkpp/register/zkppformuser1`, {
          method: 'POST',
      ...
      await t.step('POST /zkpp/register step 2 with form-urlencoded body', async () => {
        const keyPair = nacl.box.keyPair()
        const publicKey = Buffer.from(keyPair.publicKey).toString('base64url')
        const publicKeyHash = Buffer.from(nacl.hash(Buffer.from(publicKey))).toString('base64url')

        const res1 = await fetch(`${baseURL}/zkpp/register/zkppformuser2`, {
          method: 'POST',
          headers: { 'content-type': 'application/x-www-form-urlencoded' },
          body: `b=${encodeURIComponent(publicKeyHash)}`
        })
        ...
        const [, , encryptedHashedPassword] = saltsAndEncryptedHashedPassword(step1.p, keyPair.secretKey, 'testhash')
        const res2 = await fetch(`${baseURL}/zkpp/register/zkppformuser2`, {
          method: 'POST',
          headers: { 'content-type': 'application/x-www-form-urlencoded' },
      ...
      await t.step('POST /zkpp/{contractID}/updatePasswordHash with form-urlencoded body returns 400 for missing params', async () => {
        const cid = createCID('zkpp-form-test', multicodes.SHELTER_CONTRACT_DATA)
        const res = await fetch(`${baseURL}/zkpp/${cid}/updatePasswordHash`, {
          method: 'POST',
          headers: { 'content-type': 'application/x-www-form-urlencoded' },
          body: new URLSearchParams({ r: 'a', s: 'b' }).toString()
        })
        await res.body?.cancel()
        if (res.status !== 400) throw new Error(`Expected 400 but got ${res.status}`)
      })
